        ...
        if(e.target.value){
            ...
            if (searchFullindex){
                if(Number(ICDCheck)===1)
                    ...
                    let prefix = 'Neoplasm, neoplastic ';
                    if (e.target.value.startsWith('Neoplasm, neoplastic '))
                        prefix = '';
                    url='icd/neoplasm_filter/suggest/?full_title_no_nemod_suggest__completion='+prefix+e.target.value;
                }
            }
            else {
                if(Number(ICDCheck)===1)
            ...
            try {
                if(searchFullindex) {
                    const options1 = resp.full_title_no_nemod_suggest_prefix__completion_match[0].options.map((i) => ({
                        id: i._id,
                        value : i._source.full_title_no_nemod,
                        label : i._source.full_title_no_nemod
                    }));
    ...
    {   
        ...
        itemData = clieldData.map((data,index)=>{
            ...
            if(index!==(clieldData.length-1)){
                ...
                str1=<React.Fragment><span className="">{data.name}</span>{chield_btn} {seecat} {subcat} 
                {
                    ...
                    data.see.replace('— see', '').split('/').map((seeDataItem,key)=>{
                        ...
                        return (
                         <em><span className="icd_search_heading_3" data-keyword={see} onClick={manageKeyword}> {sep} {seeDataItem} </span></em>
                        )
                    })
                }
                {
                    ...
                    data.seeAlso &&
                    data.seeAlso.replace('— see also', 'See Also:').split('/').map((seeDataItem,key)=>{
                        ...
                        return (
                         <em><span className="icd_search_heading_3" data-keyword={seeAlso} onClick={manageKeyword}> {sep}{seeDataItem} </span></em>
                        )
                    })
                }</React.Fragment>;
             } 
             else
             {
                ...
                str1=<React.Fragment><span className="icd_search_sub_3">{data.name}</span>{chield_btn} {seecat} {subcat} 
                {
    ...
    {   
        ...
        itemData = clieldData.map((data,index)=>{
            ...
            if(data.d1.length>0)
            {
                ...
                str1=<li key={index}><span className="">{data.name}</span>{chield_btn} {seecat} {subcat}
                    {
            ...
            else 
            {
                ...
                str1=<li key={index}><span className="icd_search_sub_3">{data.name}</span>{chield_btn} {seecat} {subcat} 
                 {
                    ...
                    }
                    {
                        ...
                        data.seeAlso &&
                        data.seeAlso.replace('— see also', '').split('/').map((seeDataItem,key)=>{
                            seeAlso = seeAlso+' '+seeDataItem;
                                let sep = '/'
                                if (key === 0){
                                    sep = 'See also: '
...
function get_see_disp(disp_str, start_str) {
    let see = '';
    return disp_str.split('<br />').map((seeDataItem, key) => {
        let link_text = seeDataItem.split('<l>');
        let text = link_text[0];
        if (key === 0){
            text = start_str + text; 
        }
        else {
        ...
        if (link_text.length === 2) {
            link = link_text[1].replace('</l>','');
        }
        see = see + link + ' ';
        return (
            <em><span className="icd_search_heading_3" data-keyword={see} onClick={manageKeyword}> {text} </span></em>
        )
    })
}
...
function ShowICDCodeTree()
        ...
        return(<Row>  
            <Col md={6}>
                <ul className="icd_parent_list">
                    ...
                    let see_also_disp = '';
                    if (item.see_disp) {
                        see_disp=get_see_disp(item.see_disp, '- see ');
                    }
                    if (item.seeAlso_disp) {
                        see_also_disp=get_see_disp(item.seeAlso_disp, '- see also ');
                    }
                    let seecat='';
                    let subcat='';
                    let chield_btn='';
                    ...
                    if(item.path.length !== 0){
                        upButton=<span onClick={manageKeyword} data-keyword={item.path.at(-1).full_title_no_nemod} className="m1-1 icd_search_sub_3 badge"> 🡤 Up</span>; 
                    }
                    ...
                    if(item.morph !== null && item.morph){
                        morph=<span onClick={setCodeonField} data-asterisk={item.morph.name} data-code={item.morph.name} className="ml-1 icd_search_sub_2 badge">{item.morph.name}</span>; 
                    }
                    ...
                    //             see=<em><span className="icd_search_heading_2"> - see </span><span className="icd_search_heading_3" data-keyword={item.see} onClick={manageKeyword}> {item.see} </span></em>
                        return (<li key={index}><strong>{item.full_title}</strong>{chield_btn} { dagger } { asterisk } {morph} {upButton} {seecat} {subcat} 
                        {
                            item.see && see_disp
                        }
                        {
                            item.seeAlso && see_also_disp
                        }
                        <ul className="chieldList">{ manageindexICDChield(item) }</ul> </li>)
                    })}
                </ul>
            </Col>
            <Col md={6}>
                ...
                <Col md={12}>
                        ...
                        {diags && diags.map((item)=>{
                                        ...
                                        if(innerICDCode===item.name) {
                                            ...
                                            return (<TreeItem key={indexCount}  data-id={item.id} data-tag={item.name}   nodeId={`00${item.id}`}  label={<React.Fragment><div style={{'fontWeight':'bold'}} id={'parent_'+item.id}><span style={{'boxShadow':'rgb(158, 143, 143) -2px 3px 20px 10px'}} data-nodeId={indexCount} data-tag={item.name} data-id={item.id} id={item.id}  className="mt-2 mb-2 ml-2 badge badge-primary pl-1 pr-1 pt-1 pb-1 tablularInnerPopupCode">{item.name}</span> {item.desc}</div></React.Fragment>}>
       ...
       if(neoplasmICDresponse.length>0)
       {
                ...
                return(<Row> 
                    <Col md={6}>
                        <ul className="icd_parent_list">
                            ...
                            if(item.path.length !== 0 && item.path.at(-1).full_title_no_nemod !== 'Neoplasm, neoplastic'){
                                upButton=<span onClick={manageKeyword} data-keyword={item.path.at(-1).full_title_no_nemod} className="m1-1 icd_search_sub_3 badge"> 🡤 Up</span>; 
                            }
                            ...
                            return (<li key={index}><strong>{item.full_title}</strong> {upButton} 
                                {
                                    item.see && see_disp
                                }
                                {
        ...
        <Col md={6}>
            <ul className="icd_parent_list">
                ...
                if(item.c1>0) {
                    return (<li key={index}><strong>{item.name}</strong>{chield_btn} {seecat} {subcat} 
                    {
                        item.see && see_disp
                    }
                    {
                ...
                {
                    return (<li key={index}><span className="icd_search_sub_3">{item.name}</span>{chield_btn} {seecat} {subcat} 
                    {
                        item.see && see_disp
                    }
                    {
   ...
   function manageindexICDChield(item)
   {
       ...
       if(!!item.code_to && item.code_to.length>0)
       {    
            itemData = item.code_to.map((data,index)=>{
                let str1=<li key={index}>{data.desc}<span onClick={setCodeonField} data-code={data.name} className="ml-1 icd_search_sub_2 badge">{data.name}</span></li>;
                return str1;
            });
            return itemData;
       }
