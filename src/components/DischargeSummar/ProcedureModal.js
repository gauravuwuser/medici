...
export default function ProcedureModal(props) {
            ...
            itemData = clieldData.map((data, index) => {
                ...
                if (index !== (clieldData.length - 1)) {
                    ...
                    str1 = <React.Fragment><span className="">{data.name}</span>{chield_btn} {block_btn} {see_block} {
                        ...
                        data.see &&
                        data.see.replace('— see', 'See:').split('/').map((seeDataItem, key) => {
                            see = see + ' ' + seeDataItem;
                            return (
                                <em><span className="icd_search_heading_3" data-keyword={see} onClick={manageKeyword}> / {seeDataItem} </span></em>
                            )
                        })
                    }
                        {
                            ...
                            data.seeAlso.replace('— see also', 'See Also:').split('/').map((seeDataItem, key) => {
                                ...
                                return (
                                    <em><span className="icd_search_heading_3" data-keyword={seeAlso} onClick={manageKeyword}> / {seeDataItem} </span></em>
                                )
                            })
                        }
                    </React.Fragment>;
                ...
                <Col md={12}>
                    <ul className="icd_parent_list">
                        {indexICDresponse && indexICDresponse.map((item, index) => {
                            ...
                            if (item.see_also_disp) {
                                see_also_disp=get_see_disp(item.see_also_disp, '- see also');
                            }
                            ...
                            return (<li key={index}><strong>{item.full_title}</strong>{chield_btn} {block_btn} {upButton} { see_block}
    ...
    function manageindexICDChield(item) {
        ...
        {    
            itemData1 = item.code_to.map((data,index)=>{
                let str1=<li key={index}>{data.desc}
                <span onClick={setCodeonField} data-code={data.name} className="ml-1 icd_search_sub_2 badge">{data.name}</span>
                <span onClick={setCodeonField} data-code={data.name} data-block={data.block} className="ml-1 block_css badge">{data.block}</span>
                </li>;
                return str1;
            });
       }
       ...
       return <span> {itemData} <b>Code To:</b> {itemData1} </span>;
    }
    function ShowBody() {
