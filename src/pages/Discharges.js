...
if(discharges) {
    ...
    for(var i=0;i<discharges_arr.length;i++)
    {
        ...
        if(discharges_arr[i].coder_sessions.length>0){
            ...
            if(discharges_arr[i].coder_sessions[0].session_status===2)
                status=<span className="text-primary">In-Progress</span>;
            else if (discharges_arr[i].coder_sessions[0].session_status===3)
                status=<span className="text-warning">Paused </span>;
            else if (discharges_arr[i].coder_sessions[0].session_status===4)
                status=<span className="text-warning">Paused </span>;
            else if (discharges_arr[i].coder_sessions[0].session_status===5) {
                if (discharges_arr[i].coder_sessions[0].session_audit) {
                    if (discharges_arr[i].coder_sessions[0].session_audit.error === true) 
                        status=<span className="text-warning">Done-Audit </span>;
                    else 
            ...
            else if (discharges_arr[i].coder_sessions[0].session_status===6)
                status=<span className="text-success">Done-Auto </span>;
        }
        else 
        {
